...
void search(struct Pers* people, int size) {
	char poiskFam[100];
	int len, check = 0;
	system("cls");
...
void out(struct Pers* people, int size) {
	for (int i = 0; i <= size - 1; i++) {
		...
		cout << "Дата заселения:" << people[i].datezas;
...
int readFromFile(Pers* people, string fileName, int size) {
	...
	if (!fin.is_open()) {
		...
		for (int i = 0; i < n; i++) {
			...
			/*fin.getline(buff, 500);
			fin.getline(people[size - 1].f,100);
			fin.getline(people[size - 1].i,100);
			fin.getline(people[size - 1].o,100);
			//fin.getline(buff, 500);
			fin.getline(people[size - 1].komment, 100);
			fin >> people[size - 1].datezas;
			fin.getline(buff, 500); fin.getline(buff, 500);
...
Pers* delElem(struct Pers* people, int size) {
	int n;
	cout << "Введите номер элемента, который надо удалить (от 1 до " << size << " )" << endl;
	cin >> n;
	if (n >= 0 && n < size) {
		Pers* buf = new Pers[size];
		for (int i = 0; i < size; i++) {
			buf[i] = people[i];
		}
		--n;
		--size;
		people = new Pers[size];
		int q = 0;
		for (int i = 0; i <= size; i++) {
			if (i != n) {
				people[q] = buf[i];
...
int main()
{
	...
	do {
		...
		case 4:
			...
			n = readFromFile(people1, fileName, size);
			size += n;
			break;
